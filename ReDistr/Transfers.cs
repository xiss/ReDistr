...
namespace ReDistr
{
	public partial class Transfers
	{
		...
		public void FillList(List<Transfer> transfers)
		{
			...
			var unitedTransfers = Config.Config.PossibleTransfers;
			var curentRow = StartRow;
			var firstIteration = true;
			var resultRangeHeader = new dynamic[1, ItemParametrsCount + Config.Config.StockCount * 4 + Config.Config.CountPossibleTransfers];
			...
			foreach (var unitedTransfer in unitedTransfers)
			{
				...
				var resultRange = new dynamic[transferList.Count + 1, ItemParametrsCount + Config.Config.StockCount * 4 + Config.Config.CountPossibleTransfers];
				...
				foreach (var curentTransfer in transferList)
				{
					...
					y += (int)Config.Config.StockCount * 3;
				...
				Range[Cells[curentRow, 1], Cells[curentRow + transferList.Count, ItemParametrsCount + Config.Config.StockCount * 4 + Config.Config.CountPossibleTransfers]].Value2 = resultRange;
				// Применяем стили
				Range[Cells[curentRow, 1], Cells[curentRow, ItemParametrsCount + Config.Config.StockCount * 4 + Config.Config.CountPossibleTransfers]].Style = TransferNameStyle;
				Range[Cells[curentRow, ItemParametrsCount], Cells[curentRow + transferList.Count, ItemParametrsCount]].Style = CountNameStyle;
				// Границы колонок
				Range[Cells[curentRow + 1, ItemParametrsCount + 1],Cells[curentRow + transferList.Count, ItemParametrsCount + Config.Config.StockCount]].Borders(XlEdgeRight).Weight = XlThin;
				Range[Cells[curentRow + 1, ItemParametrsCount + 1],Cells[curentRow + transferList.Count, ItemParametrsCount + Config.Config.StockCount * 2]].Borders(XlEdgeRight).Weight = XlThin;
				Range[Cells[curentRow + 1, ItemParametrsCount + 1],Cells[curentRow + transferList.Count, ItemParametrsCount + Config.Config.StockCount * 3]].Borders(XlEdgeRight).Weight = XlThin;
				Range[Cells[curentRow + 1, ItemParametrsCount + 1],Cells[curentRow + transferList.Count, ItemParametrsCount + Config.Config.StockCount * 4]].Borders(XlEdgeRight).Weight = XlThin;

				curentRow += transferList.Count + 1;
			}
			// Выводим заголовки
			Range[Cells[2, 1], Cells[2, ItemParametrsCount + Config.Config.StockCount * 4 + Config.Config.CountPossibleTransfers]].Value2 = resultRangeHeader;
		}
		...
		public void MakeImportTransfers()
		{
			...
			do
			{
				...
				if (Range[ColCount + curentRow].Value2 == null)
				{
					...
					if (selectionRange != null)
					{
						ReDistr.MakeImpot1CBook(selectionRange, bookName, Config.Config.Inst.FilesCfg.FolderTransfers);
						selectionRange = null;
					}
				}
			}
